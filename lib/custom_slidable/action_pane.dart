...
class ActionPane extends StatefulWidget {
  ...
  /// A widget which animates when the [CustomSlidable] moves.
  final Widget motion;

  /// A widget which controls how the [CustomSlidable] dismisses.
  ...
  /// The fraction of the total extent from where the [CustomSlidable] will
...
class _ActionPaneState extends State<ActionPane> implements RatioConfigurator {
  ...
  void initState() {
    super.initState();
    controller = CustomSlidable.of(context);
  ...
  void handleRatioChanged() {
    final show =
        controller!.ratio.abs() <= widget.extentRatio &&
...
class _ActionPaneScope extends InheritedWidget {
  const _ActionPaneScope({this.actionPaneData, required super.child});
